...
\usepackage{pdflscape}		% pc version
%\usepackage{rotating}		% print version
...
% new table column type - flexible width + centering
...
\begin{tabularx}{7.3cm}{|r||S[table-format=2.1]|S[table-format=3.0]|C|} \hline
...
and numerous research details they never may have never asked for $\ddot\smile$.
...
testing other properties requires beginning the process of testing all over again.
...
\begin{landscape}		% pc version
\begin{table}[p]		% pc version
%\begin{sidewaystable}	% print version
\centering
\newsavebox{\temp}
\newcolumntype{CC}{>{\centering \begin{lrbox}{\temp} \arraybackslash}X<{\end{lrbox} \m{\unhbox\temp} \arraybackslash}}
\begin{tabularx}{22cm}{|c|>{\raggedright\arraybackslash}p{2.5cm}*{4}{||CC|CC|CC}|} \cline{3-14}
...
%\end{sidewaystable}	% print version
\end{table}			% pc version
\end{landscape}		% pc version
...
The ciphers were tested unlimited and then for all lower number of rounds until reaching indistinguishability from a random stream.
...
\caption{Random distinguishers for TSC-4 ciphertext. For the notes on fields marked with asterisk, 
see \autoref{sec:estream-results}.}
...
\begin{myItemize}
\item The node \textsc{1\_2\_div} located divides the value of \textsc{in\_2} by \textsc{in\_7}, \textsc{in\_12} and \textsc{in\_14}
resulting in a byte with very low Hamming weight.
\item The node \textsc{1\_3\_or} makes logical disjunction of 7 different input bytes resulting in a byte with very 
high Hamming weight.
\item The result of node \textsc{2\_2\_nor} is a negation \textsc{1\_2\_div}. Its Hamming weight should therefore be generally
very high (in 99.97\% of the tested 1\,000\,000 input sequences it produced binary one).
\item The node \textsc{2\_3\_bsl\_3} results in the last three bits of \textsc{1\_3\_or} and thus most likely outputs binary
representation of 3 (98.43\% of the tested cases).
\item The output of \textsc{3\_0\_sum} has low Hamming weight (due to overflow in addition), usually being a binary
representation of 2 (98.40\% of tested cases).
\item Nodes \textsc{2\_1\_const\_1}, \textsc{3\_1\_nor} and \textsc{3\_2\_const\_80} produce constant output.
\item The node \textsc{1\_5\_nop} takes \textsc{in\_5} without change and the node \textsc{1\_6\_ror\_1} is solely dependent 
on input byte \textsc{in\_6}.
\item The node \textsc{2\_5\_sub} computes \textsc{1\_5\_nop} - \textsc{1\_5\_nop} - \textsc{1\_6\_ror\_1} resulting in value 
solely dependent on \textsc{in\_6}.
\end{myItemize}

\noindent
Following the reasoning the the above list, we can conclude that both output bytes (\textsc{1\_out} and \textsc{2\_out})
depend solely on 7\textsuperscript{th} input byte (\textsc{in\_6}). Circuits evolved in parallel runs exhibited very similar 
behaviour -- in may of them, the output bytes (and thus the final verdict) depended only on the 7\textsuperscript{th} input byte.
It is difficult to tell what is the exact form of this weakness, but it draws out attention to the ever-mentioned byte 7.
It definitely implies a possible design flaw in Salsa20 limited to 2 rounds influencing the randomness 
of every 7\textsuperscript{th} output byte.
...
\caption{Random distinguishers for DCH output. For the notes on fields marked with asterisk, 
see \autoref{sec:hash-results}.}
\label{tab:sha3-dch}
\bigskip
\resultsTableSha{
0 & 0.0 & 0 & $n=484$ \\ \hline
...
\caption{Random distinguishers for MD6 output. For the notes on fields marked with asterisk, 
see \autoref{sec:hash-results}.}
\label{tab:sha3-md6}
\bigskip
\resultsTableSha{
