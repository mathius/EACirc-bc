...
In this chapter we analyse one selected result from the previous experiment in a more detailed manner.
...
\begin{myItemize}
\item figure 1
\item typical jaw-like shape (over-learning to specific test vector set, drop after change)
\item figure 2
\item specific for Salsa20-2: dropping value rises (learning), drops down again
\item the distinguisher stabilises and then drops -> this repeats almost periodically every ~350 KB
\item hypothesis: we can evolve distinguisher for each 350 KB, but are unable to produce a distinguisher for whole stream
\end{myItemize}

\begin{figure}[b!]
\centering
\includegraphics[width=\textwidth]{images/random2random_overlearning_jaw}
\caption{Circuit success rate for control experiment trying to distinguish two streams of quantum random data.}
\label{fig:random-jaw}
\end{figure}

\begin{figure}[t!]
\centering
\includegraphics[width=\textwidth]{images/salsa20_2rounds_jaw}
\caption{Circuit success rate for distinguishing Salsa20 limited to 2 rounds from quantum random data.}
...
\begin{myItemize}
\item analysing best individuals after 30 000 generations
\item statistics was made on outputs of individual nodes based on 1 000 000 random 16-byte input sequences
\item see figure
\item \textsc{1\_2\_DIV} divides by \textsc{IN\_7}, \textsc{IN\_12} and \textsc{IN\_14} resulting in a byte with very low Hamming weight
\item 1\_3\_OR makes logical disjunction of 7 input bytes resulting in a byte with very high Hamming weight
\item 2\_2\_NOR negates 1\_2\_DIV -> high Hamming weight (99.97\% is binary one)
\item 2\_3\_BSL\_3 results in 3 in most cases (98.43\% is binary 3)
\item 3\_0\_SUM has low Hamming weight (due to overflow in addition) (98.40\% is binary 2)
\item 1\_5\_NOP takes IN\_5 without change and 1\_6\_ROR\_1 is solely dependent on IN\_6
\item 2\_5\_SUB computes 1\_5\_NOP - 1\_5\_NOP - 1\_6\_ROR\_1 resulting in value solely dependent on IN\_6
\item constant branches in the middle
\item both output bytes are dependent only on 7th input byte (IN\_6)
\item circuits evolved in parallel runs exhibited very similar behaviour -- in may of them, the output bytes depended only
on the 7th input byte
\item => the output produced by Salsa20 reduced to 2 rounds is weaker on every 7th byte than on others
\item -> possible design flaw?
\item difficult to tell what is the exact form of weakness, but draws out attention to the ever-mentioned byte 7, so that
would be the byte we would concentrate on if improving Salsa20
\end{myItemize}
